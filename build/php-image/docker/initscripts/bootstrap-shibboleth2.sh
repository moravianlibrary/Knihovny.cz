...
main() {
    ...
    if [ -z "$PARAM_VUFIND_HOST" ]; then
        export  PARAM_VUFIND_HOST="localhost"
    fi;

    if [ -z "$PARAM_VUFIND_SSL_PORT" ]; then
        export PARAM_VUFIND_SSL_PORT="443"
    fi;

    if [ -z "$PARAM_VUFIND_URL" ]; then
        export PARAM_VUFIND_URL="https://$PARAM_VUFIND_HOST"
        if [ "$PARAM_VUFIND_SSL_PORT" != "443" ]; then
            export  PARAM_VUFIND_URL="https://$PARAM_VUFIND_HOST:$PARAM_VUFIND_SSL_PORT"
        fi;
    fi;

    if [ -z "$PARAM_VUFIND_ENTITY_ID" ]; then
        export PARAM_VUFIND_ENTITY_ID=$PARAM_VUFIND_URL
    fi;

    WAYF_FILE_URL="$PARAM_VUFIND_URL/Wayf"
    # Encode for URL parameter:
    export  WAYF_FILE_URL=$(echo "$WAYF_FILE_URL" | sed -e 's/%/%25/g' -e 's/ /%20/g' -e 's/!/%21/g' -e 's/"/%22/g' -e 's/#/%23/g' -e 's/\$/%24/g' -e 's/\&/%26/g' -e 's/'\''/%27/g' -e 's/(/%28/g' -e 's/)/%29/g' -e 's/\*/%2a/g' -e 's/+/%2b/g' -e 's/,/%2c/g' -e 's/-/%2d/g' -e 's/\./%2e/g' -e 's/\//%2f/g' -e 's/:/%3a/g' -e 's/;/%3b/g' -e 's//%3e/g' -e 's/?/%3f/g' -e 's/@/%40/g' -e 's/\[/%5b/g' -e 's/\\/%5c/g' -e 's/\]/%5d/g' -e 's/\^/%5e/g' -e 's/_/%5f/g' -e 's/`/%60/g' -e 's/{/%7b/g' -e 's/|/%7c/g' -e 's/}/%7d/g' -e 's/~/%7e/g')

    envsubst.a8m -i ${SHIB_CONF_TEMPL_ABS_PATH} -o ${SHIB_CONF_ABS_PATH}
